...
const SidebarLink = ({ href, icon: Icon, label, isCollapsed }: SidebarLinkProps) => {
    ...
    return (
        <Link href={href}>
            <div className={`cursor-pointer flex items-center ${isCollapsed ? "justify-center py-4" : "justify-start px-8 py-4"} hover:text-blue-100 gap-3 transition-colors ${isActive ? "bg-blue-200 text-white" : ""}`}>
    ...
    return (
        <div className={sidebarClassName}>
            ...
            <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
                <Image src="https://s3-bucket-inventorymanagement.s3.ap-southeast-1.amazonaws.com/logo.png" alt="logo"
                    width={27} height={27} className="rounded w-8"
                />
                <h1 className={`${isSidebarCollapsed ? "hidden" : "block"} font-extrabold text-2xl`}>YSTOCK</h1>
